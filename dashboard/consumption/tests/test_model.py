...
class UserDataTransactionTests(TransactionTestCase):
    def test_error_same_id_create(self):
        ...
        with self.assertRaisesMessage(
                IntegrityError, "UNIQUE constraint failed: consumption_userdata.id"
        ):
            UserData.objects.create(**user_data)
...
class ConsumptionDataTests(TestCase):
    ...
    def test_error_create_data_no_timedate(self):
        ...
        consumption_data = ConsumptionData(
            user=user_data, datetime=None, consumption=100.0
        )
    ...
    def test_error_create_data_invalid_consumption(self):
        ...
        user_data = UserData.objects.create(id="1000", area="a1", tariff="t1")
        consumption_data = ConsumptionData(
            user=user_data, datetime=dt, consumption=None
        )
    ...
    def test_error_same_date_create_data(self):
        """同じユーザ、同じ日時でデータが作成できないこと"""

        user_data = UserData.objects.create(id="1000", area="a1", tariff="t1")

        with self.assertRaisesMessage(IntegrityError, "UNIQUE constraint failed"):
            ConsumptionData.objects.create(
                user=user_data, datetime=dt, consumption=100.0
            )
            ConsumptionData.objects.create(
                user=user_data, datetime=dt, consumption=100.0
            )
    ...
    def test_get_data_from_user(self):
        ...
        ConsumptionData.objects.create(
            user=user_data_1, datetime=dt.replace(day=2), consumption=102.0
        )
        ConsumptionData.objects.create(
            user=user_data_1, datetime=dt.replace(day=3), consumption=103.0
        )
