...
- dataディレクトリ内に存在するデータを都度、全件削除、登録するようにしている
    - データが増えた時に、増分のみ登録する作りにはしていません

- サマリーページのユーザデータについては、ページネーションは考慮していません
...
- 既存データの全削除
    ...
    - 料金表データの登録を行う
- 消費データの前処理を行う
    - ユーザID.csv内で日時の重複があった場合、はじめのレコードを活かし、あとのレコードの削除する
    - 30分間隔のデータに時間的欠損があった場合、補完を行う
    - 加工したデータを、preprocessed_data/consumption に保存する
- ユーザデータのバリデーション
    - データ全件のチェックを行う
    - エラーがあれば、エラーメッセージの出力を行う
    - 1件でもエラーがあれば、この段階で投入処理を止める
- 消費データのバリデーション
    - データ全件のチェックを行う
    - エラーがあれば、エラーメッセージの出力を行う
    - 1件でもエラーがあれば、この段階で投入処理を止める
- ユーザデータの登録を行う
    - 全件バルクインサートを試みる
    - ユーザIDの重複があった場合、例外をキャッチしてエラーメッセージの出力を行う
- 消費データの登録を行う
    - 各ファイルで、1000件単位でバルクインサートを試みる
    - ユーザIDと日付のセットで重複があった場合、例外をキャッチしてエラーメッセージの出力を行う
- 前処理を行ったデータの削除を行う
    - preprocessed_data/consumption 内のファイルの削除を行う

### サマリーページについて

- 全ユーザを対象とした月別総消費量の表示を行う
- 全ユーザを対象とした月別1日あたりの平均消費量の表示を行う
- ユーザ一覧の表示を行う

### 詳細ページについて

- ユーザID情報の表示を行う
- ユーザIDを対象とした月別総消費量の表示を行う
- ユーザIDを対象とした月別1日あたりの平均消費量の表示を行う
...
| 論理名 | 物理名 | データ型 | not nul 制約 | PRIMARY KEY 制約 | 
| ---- | ---- | ---- | ---- | ---- |
| エリアID | id | integer | ◯ | ◯ |
| エリア名 | name | varchar(2) | ◯ |
...
| 論理名 | 物理名 | データ型 | not nul 制約 | PRIMARY KEY 制約 | ユニーク 制約 | 外部キー |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| ユーザID | id | integer | ◯ | ◯ | ◯ |
...
- Django
- django-pandas
    - モデルのクエリセットをpandasのDataFrameに変換するために使用する
- isort
    - import の並べ替えのために使用する
- flake8
    - コードのスタイルチェックのために使用する
- Black
    - コードの自動的に整形のために使用する
- Bandit
    - コードのセキュリティチェックのために使用する
- Pandas (https://pandas.pydata.org/)
    - CSVデータの内のデータ重複削除、欠損データ補完に使用する
- jupyter (https://jupyter.org/)
　   - Pandasでのデータ前処理を検証するために使用する
...
## 折れ線グラフを描画するためのデータの受け渡しについて

- 時間の都合、View側でJSONデータを作り、テンプレート側に渡しています。
    - 本来AjaxでのAPI連携でJSONデータのみの受け渡しが適切だと思います。
...
- サマリーページの表示の遅さ
    - データの多さからくる総消費量、平均値の算出に時間がかかっている
    - 予め総消費量、平均値を算出してデータベースに保存しておいても良かった。
        - 実運用であれば、総消費量、平均値は日次バッチで前日分のものを集計してもちいても良い類の値だと思う。
